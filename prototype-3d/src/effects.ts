...
export function createQingyuanHitBurst(position: THREE.Vector3, direction: THREE.Vector3, elite = false, tier = 1): Effect {
  const root = new THREE.Group();
  ...
  const color = tier >= 3 ? 0xffe9b0 : tier >= 2 ? 0xcdeaff : elite ? 0xdffef6 : 0xb8fff0;
  const shardN = 12 + (tier - 1) * 8;
...
export function createThunderStrike(position: THREE.Vector3, tier = 1): Effect {
  const root = new THREE.Group();
...
export function createBindingField(position: THREE.Vector3, radius = 4.2, tier = 1): Effect {
  const root = new THREE.Group();
  root.name = "Effect_BindingArray";
  root.position.copy(position);

  const ringColor = tier >= 3 ? 0xffe0b0 : tier >= 2 ? 0xb8e8ff : 0xdcb8ff;
  const ringEmis = tier >= 3 ? 0xffbf4a : tier >= 2 ? 0x5ec8ff : 0x9f7aff;
  const ring = new THREE.Mesh(new THREE.TorusGeometry(radius, 0.035 + (tier - 1) * 0.02, 8, 72), makeMat(ringColor, ringEmis, 1.8 + (tier - 1) * 0.4, 0.62));
  ring.rotation.x = Math.PI / 2;
  ...
  for (let i = 0; i < pillarN; i++) {
    const pillar = new THREE.Mesh(new THREE.BoxGeometry(0.08, 1.25, 0.08), makeMat(0xf1e8ff, ringEmis, 1.7, 0.58));
    const angle = (i / pillarN) * Math.PI * 2;
